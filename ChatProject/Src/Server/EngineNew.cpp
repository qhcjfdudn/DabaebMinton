...
EngineNew& EngineNew::GetInstance() {
	static EngineNew instance;
	return instance;
}

void EngineNew::TurnOff()
{
	isRunning = false;
	observer.notify(ObserverEvent::EngineOff);
}

void EngineNew::initPhysics()
{
...
const PxTolerancesScale& EngineNew::GetTolerancesScale() const
{
	return pxPhysics->getTolerancesScale();
}

PxCpuDispatcher* EngineNew::GetCpuDispatcher()
{
	return pxDispatcher;
}

PxScene* EngineNew::CreateScene(PxSceneDesc sceneDesc)
{
	return pxPhysics->createScene(sceneDesc);
}

PxRigidStatic* EngineNew::CreatePlain(float nx, float ny, float nz, float distance)
{
...
PxRigidDynamic * EngineNew::CreateBox(const PxTransform& tp, float halfExtentX, float halfExtentY, float halfExtentZ)
{
...
PxRigidDynamic * EngineNew::CreateBox2D(const PxVec2& location, float halfExtentX, float halfExtentY)
{
...
PxRigidStatic* EngineNew::CreateBox2DStatic(const PxVec2& location, float halfExtentX, float halfExtentY)
{
...
PxRigidDynamic * EngineNew::CreateSphere2D(const PxVec2& location, const PxVec2& velocity, float halfExtentRadius)
{
	PxRigidDynamic* body = pxPhysics->createRigidDynamic(PxTransform{ location.x, location.y, 0 });
...
PxRigidDynamic * EngineNew::createDynamic(const PxTransform& t, const PxGeometry& geometry, const PxVec3& velocity)
{
